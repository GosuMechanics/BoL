...
function Combo(unit)
    ...
    if unit ~= nil and unit.type == myHero.type then
            ...
            if Settings.combo.useE and GetDistanceSqr(myHero, target) > math.pow(myHero.range + myHero.boundingRadius + 50, 2) then
                    ...
                    if IsValidTarget(object) then
                        ...
                        elseif GetDistanceSqr(myHero, target) > math.pow(Settings.combo.DistanceToE, 2) then
                            CastSpell(ESpell.Slot, object)
                        end
                    end
            end
...
function Menu()
    ...
    Settings:addSubMenu("["..myHero.charName.."] - Target Selector Settings", "TS")
        ts.name = "Gosu"
        Settings.TS:addTS(ts)
        Settings.TS:addParam("Combo", "Range for Combo", SCRIPT_PARAM_SLICE, 1200, 150, 1200, 0)
        Settings.TS:addParam("Harass", "Range for Harass", SCRIPT_PARAM_SLICE, 1000, 150, 1200, 0)
...
function GetRange()
    if OrbwalkManager:IsCombo() then return Settings.TS.Combo
    elseif OrbwalkManager:IsHarass() then return Settings.TS.Harass
    else return Settings.TS.Combo
    end
end
